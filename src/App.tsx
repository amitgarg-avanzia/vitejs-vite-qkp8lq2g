...
/**
 * AVANZIA GLOBAL - EXPENSE TRACKER (Production v1.2 - Fixed Styling Race Condition)
 */
...
const EXPENSE_HEADS = [
  'Travel - Air Fare/Train',
  'Travel - Cab',
  'Lodging - Hotel Stay',
  'Boarding - Food Expenses',
  'Entertainment Expenses',
  'Local Travel - Cab/Other',
...
const PAYMENT_MODES = [
  ...
  'Paid by Company Bank Account',
  'Paid by Self / Partner',
];
...
// FORCE LOAD STYLING (Improved: Waits for window.tailwind object)
const useTailwindLoader = () => {
  const [ready, setReady] = useState(false);

  useEffect(() => {
    // 1. Check if Tailwind is already available globally (e.g. from previous load)
    if (window.tailwind) {
      setReady(true);
      return;
    }

    // 2. Check if script tag exists but maybe hasn't finished initializing
    ...
    if (existingScript) {
      const interval = setInterval(() => {
        if (checkTailwind()) clearInterval(interval);
      }, 100);
      return () => clearInterval(interval);
    }

    // 3. Inject script if completely missing
    const script = document.createElement('script');
    script.src = 'https://cdn.tailwindcss.com';
    script.async = true;
    script.onload = () => {
      // Even after onload, sometimes the object isn't instantly ready on the window
      const interval = setInterval(() => {
        if (checkTailwind()) clearInterval(interval);
      }, 50);
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Aggressive compression to ensure we stay under Firestore 1MB limit
        const MAX_WIDTH = 800;
...
const Button = ({
  ...
  const baseStyle =
    'px-4 py-2 rounded-md font-medium transition-colors flex items-center justify-center gap-2 shadow-sm';
  const variants = {
    primary: 'bg-blue-600 text-white hover:bg-blue-700 disabled:bg-blue-300',
    secondary:
      'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 disabled:bg-gray-50',
...
const ReportsView = ({ expenses, clients, generatePDF, generateExcel }) => {
  ...
  // Strict Filter for PDF generation
  const reportData = expenses.filter((item) => {
    if (filterType === 'client') {
      if (item.category !== 'billable') return false; // Only billable
      return item.client === selectedEntity;
    } else {
      if (item.paymentMode !== 'Paid by Self / Partner') return false; // Only reimbursements
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200">
        ...
        <div className="p-4 bg-blue-50 rounded-lg mb-6 text-sm text-blue-800">
          <strong>Note:</strong> "Client Invoice" only includes <u>Billable</u>{' '}
          expenses. "Partner Reimbursement" only includes expenses paid by{' '}
          <u>Self</u>.
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            ...
            >
              <option value="client">
                Client Invoice Support (Billable Only)
              </option>
              <option value="partner">Partner Reimbursement</option>
            </select>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Select {filterType === 'client' ? 'Client' : 'Partner'}
            </label>
            <select
              className="w-full border p-2 rounded-md bg-white"
          ...
          <div className="flex items-end">
            <div className="bg-gray-100 p-2 rounded-md w-full border text-center">
        ...
        <div className="flex gap-4">
          <Button
            ...
            onClick={() =>
              generatePDF(
                reportData,
                filterType === 'client'
                  ? `Billable Expenses - ${selectedEntity}`
                  : `Reimbursement Claim - ${selectedEntity}`
              )
            }
          >
          ...
          <Button
            ...
            disabled={reportData.length === 0}
            onClick={() =>
              generateExcel(reportData, `Report_${selectedEntity}`)
            }
          >
      ...
      <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 overflow-hidden">
        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
          <Table className="w-5 h-5 text-gray-600" /> Recent Activity (All
          Categories)
        </h3>
    ...
    <div className="bg-white p-6 rounded-xl shadow-md border border-gray-200 max-w-2xl mx-auto">
      ...
      <div>
        <h3 className="font-semibold text-gray-700 mb-3">
          All Expense Records (Master List)
        </h3>
            ...
            <tbody className="divide-y">
              {expenses.map((exp) => (
                <tr key={exp.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-2">{exp.partner}</td>
        ...
        <div className="mt-4">
          <Button
            variant="outline"
            className="w-full"
            onClick={() => generateExcel(expenses, 'Master_Export')}
          >
            <Download className="w-4 h-4" /> Download Complete Master Data
            (Excel)
          </Button>
        </div>
      </div>
...
export default function AvanziaExpenseTracker() {
    ...
    const initAuth = async () => {
      try {
        ...
        if (
          ...
          setAuthError(
            "ACTION REQUIRED: Enable 'Anonymous' sign-in in Firebase Console."
          );
        } else {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const qClients = query(
        collection(db, 'artifacts', appId, 'public', 'data', 'avanzia_clients')
      );
      const unsubClients = onSnapshot(qClients, (snapshot) => {
        if (!snapshot.empty) {
          const loadedClients = snapshot.docs.map((doc) => doc.data().name);
          if (loadedClients.length > 0)
            setClients([...new Set([...INITIAL_CLIENTS, ...loadedClients])]);
        }
      });
      return () => {
        unsubExpenses();
  ...
  const showNotification = (message, type = 'success') => {
    ...
    setTimeout(() => setNotification(null), 5000); // 5s for better reading
  };
  ...
  const handleFileUpload = async (e) => {
    ...
    // Check size BEFORE processing (1MB limit for Firestore)
    if (file.size > 1024 * 1024) {
      showNotification(
        'File too large. Please use an image/pdf under 1MB.',
        'error'
      );
      return;
    }
    ...
    if (file.type === 'application/pdf') {
      try {
        ...
        showNotification('Error reading PDF', 'error');
      }
  ...
  const handleAddClient = async (newClientName) => {
    ...
    try {
      await addDoc(
        collection(db, 'artifacts', appId, 'public', 'data', 'avanzia_clients'),
        { name: newClientName, createdAt: new Date().toISOString() }
      );
      showNotification(`Client ${newClientName} added.`);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!user) {
      showNotification(
        'Authentication missing. Refresh page or check Console.',
        'error'
      );
      return;
    }

    // Final check on payload size estimate
    if (formData.receiptImage && formData.receiptImage.length > 1048487) {
      showNotification(
        'Compressed image is still too large for database. Try a smaller file.',
        'error'
      );
      return;
    }

    if (!formData.receiptImage && !formData.missingReceiptReason) {
      showNotification('Please attach a bill OR provide a reason.', 'error');
      return;
    }
    ...
    try {
      ...
      // Detailed Error Reporting for User
      if (error.code === 'permission-denied') {
        showNotification(
          'Permission Denied: Go to Firestore Database > Rules > Allow read/write.',
          'error'
        );
      } else if (
        error.message &&
        ...
        showNotification('File too large for database (Max 1MB).', 'error');
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteDoc(
        doc(db, 'artifacts', appId, 'public', 'data', 'avanzia_expenses', id)
      );
  ...
  // ... (PDF/Excel Logic remains same as previous working version)
  const generatePDF = async (data, title) => {
    if (!window.jspdf || !window.PDFLib) {
      showNotification(
        'Libraries loading... please wait 5s and try again.',
        'error'
      );
      return;
    }
    ...
    try {
      ...
      doc.text('Avanzia Global Private Limited', 14, 20);
      doc.setFontSize(12);
      ...
      data.forEach((item) => {
        ...
        tableRows.push([
          ...
          item.client || '-',
          item.description,
          item.paymentMode === 'Paid by Self / Partner' ? 'Self' : 'Company',
          formatCurrency(item.amount),
        ]);
      });
      tableRows.push([
      ...
      for (const item of data) {
        ...
        if (isPdf) {
          const receiptPdfBytes = await fetch(item.receiptImage).then((res) =>
            res.arrayBuffer()
          );
          ...
          page.drawText(
            `Receipt: ${item.description} - ${formatCurrency(item.amount)}`,
            { x: 50, y: page.getHeight() - 50, size: 12 }
          );
  ...
  const generateExcel = (data, filename) => {
    if (!window.XLSX) {
      showNotification('Excel Library loading... wait 5s.', 'error');
      return;
    }
    ...
    const ws = XLSX.utils.json_to_sheet(
      data.map((item) => ({
        ...
        Amount: item.amount,
        GST_Component: item.gstAmount || 0,
        Debited_Base: item.category.includes('partner')
          ? item.amount - (item.gstAmount || 0)
          : item.amount,
        Payment_Mode: item.paymentMode,
        Receipt_Type: item.receiptImage
          ? item.receiptImage.startsWith('data:application/pdf')
            ? 'PDF'
            : 'Image'
          : 'Missing',
  ...
  const renderEntryForm = () => (
    <div className="bg-white rounded-xl shadow-md border border-gray-100 p-6 max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold text-gray-800 mb-6 flex items-center gap-2">
        <Plus className="w-6 h-6 text-blue-600" /> New Expense Entry
      </h2>
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          {(formData.category.includes('partner') ||
            formData.paymentMode.includes('Self')) && (
            <div className="bg-blue-50 p-3 rounded-md border border-blue-200">
              ...
              <input
                type="number"
                step="0.01"
                className="w-full border border-blue-200 p-2 text-sm rounded bg-white"
          ...
          <div className="md:col-span-2 border-t border-gray-200 pt-4">
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start">
              <div className="flex-1 w-full">
                <label className="flex flex-col items-center px-4 py-6 bg-white text-blue rounded-lg shadow-sm border border-blue-200 cursor-pointer hover:bg-blue-50 transition-colors">
                ...
                {!formData.receiptImage ? (
                  <div>
                    <label className="block text-xs font-bold text-red-600 uppercase mb-1">
                      No attachment? Provide reason:
                    </label>
                    <textarea
                      className="w-full border border-red-200 bg-red-50 rounded-lg p-2 text-sm"
                      rows={3}
                      value={formData.missingReceiptReason}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          missingReceiptReason: e.target.value,
                        })
                      }
                    />
                  </div>
                ) : (
                  <div className="relative h-24 w-full bg-gray-50 rounded-md border border-gray-200 flex items-center justify-center">
                    {formData.receiptImage.startsWith(
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-gray-900 pb-20">
      ...
      {notification && (
        <div
          className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-lg shadow-lg text-white font-medium animate-bounce flex items-center gap-2 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Consulting Expense Tracker
                </p>
              </div>
            </div>
            <nav className="hidden md:flex space-x-1">
              {[
                { id: 'entry', icon: Plus, label: 'New Expense' },
                { id: 'reports', icon: PieChart, label: 'Reports & Exports' },
                { id: 'admin', icon: Settings, label: 'Admin / Master' },
              ].map((item) => (
                <button
                  key={item.id}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 flex justify-around p-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        {[
          { id: 'entry', icon: Plus, label: 'Add' },
